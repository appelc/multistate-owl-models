...
## (03b) 'R' submodel: p(NOISE + EFFORT) delta(.) psi(.) R(STUDY_AREA + NR_SUIT500 + BO_TOTAL)
...
## set model name here
  ...
  modname = '03b_submodel_R_spikeSlab'
...
jdata <- list(nhex = as.numeric(nrow(det_matrix)), nVisits = as.numeric(ncol(det_matrix)), 
              ...
              SpikeMuBeta2 = c(NA, spike_mu_beta2[2:length(spike_mu_beta2)]), #replace intercept with NA
              SpikeSDBeta2 = c(NA, spike_SD_beta2[2:length(spike_SD_beta2)])) #replace intercept with NA
...
#set initial values (don't provide inits for beta2):
  inits<-function(){list(Occ.hex=y2, 
                         beta=0, gamma=rep(0,ncov_p+2), 
                         iota=rep(0,ncov_delta+1), w=rep(1,ncov_r))}
...
## run JAGS ####
  ...
  output <- jagsUI(data = jdata, inits = inits, parameters = params, store.data = TRUE,
                   model = 'bugs/03b_submodel_R_spikeSlab.bugs',
...
## check indicator variable combinations from spike-and-slab run:
  ...
  table(mod)
  sort(round(table(mod)/length(mod), 3)) #top combo for "R"
