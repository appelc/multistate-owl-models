...
## (01a) 'p' submodel: p(NOISE + EFFORT + SEASON + BO_WEEKLY) delta(.) psi(.) R(.)
...
## set model name here
  ...
  modname = '01a_submodel_p_initial'
...
params<-c('beta','beta2','gamma','iota')         ## initials
...
#set initial values:
  inits<-function(){list(Occ.hex=y2, #use highest observed state
...
## run JAGS ####
  ...
  output <- jagsUI(data = jdata, inits = inits, parameters = params, store.data = TRUE,
                   model = 'bugs/01a_submodel_p_initial.bugs',
...
## save the mean and SD of initial run:
  ...
  saveRDS(output$mean$gamma, file = paste(filedir, 'spike_gamma_mu.RDS', sep = '/'))
  saveRDS(output$sd$gamma, file = paste(filedir, 'spike_gamma_SD.RDS', sep = '/'))
