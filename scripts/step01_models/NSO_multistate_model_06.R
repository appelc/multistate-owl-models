...
## model 06: p(NOISE + EFFORT) delta(.) psi(NR_SUIT500 + BO_TOTAL) R(NR_SUIT500 + BO_TOTAL)
## post-hoc model for interpretation of covariate effects on psi and R
## but with 'STUDY_AREA' covariate omitted from psi and R to make marginal plots
## with 'normal1' priors: dnorm(0,0.368)
...
## set model name here
  ...
  modname = '06_model_posthoc_norm1priors_marginals'
...
params <-c('beta','beta2','gamma','iota',             
           'psiD','RD','occNoPair','occPair','unOcc',
           'pOcc','pOcc2','pOcc3','pOcc4','pOcc5','pOcc6',
           'pPair','pPair2', 'pPair3', 'pPair4', 'pPair5', 'pPair6',
           'condPair','condPair2','condPair3','condPair4','condPair5','condPair6',
           'truePair','truePair2','truePair3','truePair4','truePair5','truePair6')
...
#set initial values:
  inits<-function(){list(Occ.hex=y2,
                         beta=rep(0,ncov_psi+1), beta2=rep(0,ncov_r+1), 
...
## run JAGS ####
  ...
  output <- jagsUI(data = jdata, inits = inits, parameters = params, store.data = TRUE,
                   model = 'bugs/06_model.bugs',
            		   n.chains=3, n.thin=1, n.iter=100000, n.burnin=50000, n.adapt=50000, parallel=TRUE)
  (end = Sys.time() - start)
