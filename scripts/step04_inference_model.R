...
## 4) Create marginal plots (detection)  *FIG 3*
...
## 8) Create marginal plots (occupancy)  *FIG 5 (area only)*
...
## NOISE ####
  ...
  #set up matrices
  f1 <- matrix(nrow = nrow(simsNoise), ncol = length(noiseScaled)) #p1
  f2 <- matrix(nrow = nrow(simsNoise), ncol = length(noiseScaled)) #p2
  
  # multiply intercept by 1; slope by value of the covariate
  for(i in 1:nrow(simsNoise)){
    for(j in 1:length(noiseScaled)){
      f1[i, j] <- sum(simsNoise[i, ] * c(1, 0, noiseScaled[j])) #non-pair (p1)
      f2[i, j] <- sum(simsNoise[i, ] * c(1, 1, noiseScaled[j])) #pair (p2)
    }
  }
...
#EFFORT ####  
  ...
  #set up matrices
  g1 <- matrix(nrow = nrow(simsEffort), ncol = length(effortScaled)) #p1
  g2 <- matrix(nrow = nrow(simsEffort), ncol = length(effortScaled)) #p2
  
  #multiply intercept by 1; slope by value of the covariate
  for(i in 1:nrow(simsEffort)){
    for(j in 1:length(effortScaled)){
      g1[i, j] <- sum(simsEffort[i, ] * c(1, 0, effortScaled[j])) #non-pair (p1)
      g2[i, j] <- sum(simsEffort[i, ] * c(1, 1, effortScaled[j])) #pair (p2)
    }
  }
...
## NOISE
  nnInf_original <- ggplot(noisePlotinference, aes(x, y)) +
  ...
  #rename labels to just 'p1' and 'p2'
  noisePlotinference$grp2 <- ifelse(noisePlotinference$grp %in% 'Landscape\nuse', 'p1', 'p2')
  
  #plot
  nnInf <- ggplot(noisePlotinference, aes(x, y)) +
    geom_ribbon(aes(ymin = lo, ymax = hi, fill = grp2), alpha = 0.3) +
    geom_line(size = 1.5, aes(color = grp2, linetype = grp2)) +
    ylab('Weekly detection probability (p) \u00B1 95% CI') + xlab('Noise (dBFS)') +
    #scale_x_continuous(breaks = seq(0, 8000, 2000), limits = c(0, 5000)) +
    ylim(c(0,1)) +
    geom_rug(data = noiseRaw, mapping = aes(x = noise), inherit.aes = FALSE) +
    scale_fill_manual(values = c('p1' = 'black', 'p2' = 'darkblue')) +
    scale_colour_manual(values = c('p1' = 'black', 'p2' = 'darkblue')) +
    # scale_linetype_manual(values = c('p1' = 'solid', 'p2' = 'twodash')) +
    theme(panel.background = element_rect(fill = 'transparent'),
          axis.line = element_line(),
          axis.title = element_text(size = 18),
          strip.text = element_text(size = 16),
  ...
  ## Predict a couple values for reporting:
    ...
    noisePlotinference$y[noisePlotinference$x == min(noisePlotinference$x) & 
                         noisePlotinference$grp %in% 'Landscape\nuse']        #0.13 for non-pairs
    noisePlotinference$y[noisePlotinference$x == min(noisePlotinference$x) & 
                         noisePlotinference$grp %in% 'Pair\noccupancy']       #0.45 for pairs
    ...
    noisePlotinference$y[noisePlotinference$x == noiseMeanIndex & 
                         noisePlotinference$grp %in% 'Landscape\nuse']        #0.03 for non-pairs
    noisePlotinference$y[noisePlotinference$x == noiseMeanIndex & 
                         noisePlotinference$grp %in% 'Pair\noccupancy']       #0.16 for pairs
...
## EFFORT
  ...
  effortPlotinference$y[effortPlotinference$x == 3360 &
                        effortPlotinference$grp %in% 'Landscape\nuse']  #0.04 for non-pairs
  effortPlotinference$y[effortPlotinference$x == 3360 &
                        effortPlotinference$grp %in% 'Pair\noccupancy'] #0.19 for pairs
...
## Combine and export
  ...
  ## FOR FIGURE 3
  tiff(filename = 'figures/fig3.tif', height = 18, width = 16, units = 'cm',
...
## -----------------------------------------------------------------------------  
## 5) Explore covariate effects on use (psi) ####
...
## -----------------------------------------------------------------------------
## 7) Calculate predicted/fitted values (use) ####
...
## STUDY AREA ####
  ...
  for(i in 1:nrow(simsAreaPsi)){
    for(j in 1:length(areaScaled)){
      m1[i, j] <- sum(simsAreaPsi[i, ] * c(1, areaScaled[j]))  #m1: psi
    }
  }
  for(i in 1:nrow(simsAreaR)){
    for(j in 1:length(areaScaled)){
      m2[i, j] <- sum(simsAreaR[i, ] * c(1, areaScaled[j]))    #m2: R
    }
  }
  ...
  pairBTarea  <- psiBTarea * rBTarea         #prob occ by pairs (psi*R)
  nopairBTarea <- psiBTarea * (1 - rBTarea)  #prob occ by non-pair (psi * (1-R))
  # unoccBTarea <- 1 - occBTarea             #prob unoccupied (1 - psi)
  
  
  #set up dataframe
  areaPlotInf <- data.frame(x = rep(areaScaled,3),
                           y = c(apply(psiBTarea, 2, mean), 
                                 apply(pairBTarea, 2, mean), 
                                 apply(nopairBTarea, 2, mean)),
                           lo = c(apply(psiBTarea, 2, quantile, probs = 0.025), 
                                  apply(pairBTarea, 2, quantile, probs = 0.025),
                                  apply(nopairBTarea, 2, quantile, probs = 0.025)),
                           hi = c(apply(psiBTarea, 2, quantile, probs = 0.975), 
                                  apply(pairBTarea, 2, quantile, probs = 0.975), 
                                  apply(nopairBTarea, 2, quantile, probs = 0.975)),
                           grp = c(rep('Landscape\nuse', length(areaScaled)), 
                                   rep('Pair\noccupancy', length(areaScaled)),
                                   rep('Non-pair\nuse', length(areaScaled))))
...
## STUDY AREA 
  ...
  #plot
  aa <- ggplot(areaPlotInf[areaPlotInf$grp %in% c('Landscape\nuse', 'Pair\noccupancy'),], 
               aes(x, y, linetype = grp)) +
    geom_pointrange(aes(ymin = lo, ymax = hi, color = grp, shape = grp), 
                    size = 1, position = position_dodge(width = 0.3)) +
    ylab('Probability \u00B1 95 CI') + xlab('Study Area') +
    ylim(c(0,0.5)) +
    scale_shape_manual(values = c('Landscape\nuse' = 16, 'Pair\noccupancy' = 17)) +
    scale_colour_manual(values = c('Landscape\nuse' = 'black', 'Pair\noccupancy' = 'darkblue')) +
    scale_linetype_manual(values = c('Landscape\nuse' = 'solid', 'Pair\noccupancy' = 'dashed')) +
    theme(panel.background = element_rect(fill = 'transparent'),
          axis.line = element_line(),
...
## Format margins to match other figures
  ...
  ## FOR FIGURE 5
  tiff(filename = 'figures/fig5.tif', height = 13, width = 17, units = 'cm',
       res = 600, compression = 'lzw')
