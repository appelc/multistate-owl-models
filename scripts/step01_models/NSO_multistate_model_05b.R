...
## model 05b: p(NOISE + EFFORT) delta(.) psi(STUDY_AREA + NR_SUIT500 + BO_TOTAL) R(STUDY_AREA + NR_SUIT500 + BO_TOTAL)
## post-hoc model for interpretation of covariate effects on psi and R
## with 'normal2' priors: dnorm(0,0.5)
...
## set model name here
  ...
  modname = '05b_model_posthoc_norm2priors'
...
jdata <- list(nhex = as.numeric(nrow(det_matrix)), nVisits = as.numeric(ncol(det_matrix)), 
              y = det_matrix,
              area = area_covar, bo_total = bo_total_covar, suit_mean = nr500_mean, 
